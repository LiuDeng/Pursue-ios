...
@interface DetailViewController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
- (void)layoutSubviews {
    ...
    frame.size.width = CGRectGetWidth(self.view.frame);
    frame.size.height = screenHeight;//CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame));
...
- (UIColor *)indicatorColor {
    ...
    if (!_indicatorColor) {
        UIColor *color = [UIColor lightColor];//[UIColor colorWithRed:178.0/255.0 green:203.0/255.0 blue:57.0/255.0 alpha:0.75];
...
- (UIColor *)contentViewBackgroundColor {
    ...
    if (!_contentViewBackgroundColor) {
        UIColor *color = [UIColor whiteColor]; //[UIColor colorWithRed:248.0/255.0 green:248.0/255.0 blue:248.0/255.0 alpha:0.75];
        if ([self.delegate respondsToSelector:@selector(viewPager:colorForComponent:withDefault:)]) {
...
- (void)setOffset:(CGPoint)offset{
    ...
    [self.tabsView setHidden:NO];
    [self.contentView setHidden:NO];
    
    double screenWidth = [UIScreen mainScreen].bounds.size.width;
//    double screenHeight = [UIScreen mainScreen].bounds.size.height;
    _mainScrollView.contentSize = CGSizeMake(screenWidth, offset.y + self.tabsView.frame.size.height + self.contentView.frame.size.height);
}
...
- (void)selectTabAtIndex:(NSUInteger)index {
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        TabView *tv = [self tabViewAtIndex:i];
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        [self.tabsView setHidden:YES];
        
        [_mainScrollView insertSubview:self.tabsView atIndex:0];
    }
    ...
    if (!self.contentView) {
        ...
        [_mainScrollView insertSubview:self.contentView atIndex:0];
    }
